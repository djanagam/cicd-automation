Sure, I can help you with that. Let's add an option to pass the `$secret_name` as input to the script when you want to add a new registry to the Docker JSON secret.
...
```sh
./k8s_utility.sh add-registry alias1 '{"new-registry.com": {"auth": "new-auth-token"}}' my-docker-secret ns